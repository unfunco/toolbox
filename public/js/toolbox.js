...
const hasPinSearchUi =
  searchInput !== null || results !== null || status !== null
...
if (hasPinSearchUi) {
  ...
  function renderResults(query) {
    ...
    setStatus(
      `${matches.length} match${plural} for "${query.trim()}".${suffix}`,
    )
  }
  ...
  loadPins().catch((error) => {
    ...
    setStatus(
      'Pinned actions could not be loaded. Refresh and try again.',
      true,
    )
  })
}
